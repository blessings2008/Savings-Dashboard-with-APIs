  ...
  send_message: {
    ...
    execute: async (args, adminMeta) => {
      ...
      if (broadcast) {
        ...
        await batch.commit();
        await logAIAction('send_message', { broadcast: true, sent, message, topic }, adminMeta);
      ...
      await batch.commit();
      await logAIAction('send_message', { uids, sent: uids.length, message, topic }, adminMeta);
...
You respond ONLY in strict JSON, one of these three shapes — no markdown, no prose outside the JSON:
...
- You may need multiple tool_calls in sequence before you have enough to answer — that's fine, request one at a time.`;
  ...
  try {
    for (let i = 0; i < MAX_ITERATIONS; i++) {
      ...
      finalResult = { type: 'answer', text: parsed.text || raw.trim() };
      break;
    }
    ...
    res.json({ success: true, answer: finalResult.text });
  } catch (err) {
