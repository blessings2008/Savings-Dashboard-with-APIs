...
const CURRENT_TERMS_VERSION = '2026-06-01';
...
async function fetchWithRetry(url, options = {}, { timeoutMs = 15000, maxRetries = 2 } = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      clearTimeout(timer);
      // Only retry on server errors (5xx) — never on 4xx, which
      // represents a request the provider actively rejected and
      // retrying could mean double-submitting a payment attempt
...
// PocketVault's answer to an Airtel Money agent code — a short
// 5-digit number any PocketVault user can enter to pay a Business-
// plan merchant directly, without needing the merchant's phone
// number. This is the concrete "reason to upgrade" feature: lower
// fees alone are a rational reason, a code you can put on a
// signboard is a tangible one.
//
// Called from every place a user can end up on the Business plan
// (subscription payment success — both mock and real — and the AI
// assistant's change_user_plan action) so the code always exists
// regardless of how someone became Business. Idempotent: if the
// user already has a code from a previous Business stint, that same
// code is reactivated rather than a new one generated, so a
// downgrade-then-upgrade doesn't silently change a merchant's
// published code out from under them.
// ----------------------------
async function ensureMerchantCode(uid) {
  const userSnap = await db.collection('users').doc(uid).get();
  const user = userSnap.data() || {};

  if (user.merchantCode) {
    // Already has a code from a previous Business stint — just
    // reactivate it rather than generating a new one
  ...
  // Generate a new 5-digit code, retrying on the rare collision.
  // 90,000 possible codes (10000-99999) is comfortably more than
  // enough headroom for a long time at PocketVault's realistic scale.
  let code, attempts = 0;
  do {
  ...
  if (attempts >= 10) {
    // Extremely unlikely given the code space, but fail loudly
    // rather than silently assigning a duplicate
...
// Deactivates (but does not delete) a merchant code when a user
// leaves the Business plan — the code stops working for payers
// immediately, but is preserved so upgrading again later restores
// the same published code rather than issuing a new one.
async function deactivateMerchantCode(uid) {
  await db.collection('users').doc(uid).update({ merchantCodeActive: false }).catch(() => {});
}

// ----------------------------
// IDEMPOTENCY PROTECTION
// PRODUCTION FIX #1 (critical): before this, nothing stopped a
// double-tap, a slow-network retry, or a frontend bug from firing
// /api/save or /api/withdraw twice for what the user intended as
// ONE action — meaning two real charges for one intended save.
//
// The frontend generates a random idempotencyKey once, the moment
// the user taps Save/Withdraw (NOT regenerated on retry), and sends
// it with the request. The first request with a given key processes
// normally and its result is cached under that key. Every subsequent
// request with the SAME key — whether a genuine retry or an
// accidental double-submit — gets back the ORIGINAL result instead
// of being processed again, at any point in the following 24 hours.
//
// Keys are scoped per-user (a key collision across two different
// users is treated as unrelated, since they're different people's
// money) and use Firestore's atomic document-creation semantics
// (fails if the doc already exists) as the actual lock — this is
// safe even if two requests with the same key arrive at nearly the
// exact same instant.
// ----------------------------
async function withIdempotency(uid, idempotencyKey, handler) {
  if (!idempotencyKey) {
    // No key provided — caller didn't opt into idempotency protection
    // (e.g. an older client). Proceed without it rather than blocking
    // the request, but this should be treated as a temporary allowance
    // while every frontend call site is updated to always send a key.
    return handler();
  }

  const lockRef = db.collection('idempotency_keys').doc(`${uid}_${idempotencyKey}`);

  // Try to atomically claim this key. Firestore's create() fails if
  // the document already exists, which is exactly the atomic
  // test-and-set behavior needed here.
  try {
    ...
    // Document already existed — someone already claimed this key.
    // Wait briefly for the original request to finish, then return
    // its cached result rather than reprocessing.
    for (let i = 0; i < 10; i++) {
      await new Promise(r => setTimeout(r, 500));
  ...
  // We hold the lock — actually run the handler
  try {
...
// ----------------------------
async function sendExternalAlert(title, details) {
...
// ----------------------------
// HEALTH CHECK
// Upgraded to genuinely verify Firestore connectivity (a lightweight
// read) rather than always returning "ok" regardless of whether the
// database is actually reachable — point an external uptime monitor
// (UptimeRobot, Better Uptime, etc — most have a free tier) at this
// endpoint for real external monitoring of the app.
// ----------------------------
...
app.post('/api/profile',
  ...
  requireOwnData,
  asyncHandler(async (req, res) => {
    const { uid, name, phone, termsAccepted } = req.body;
    const updates = {
      name: name || null,
      phone: phone || null,
...
// ----------------------------
app.post('/api/account/accept-terms', requireAuth, requireOwnData, asyncHandler(async (req, res) => {
  const { uid } = req.body;
  await db.collection('users').doc(uid).set({
...
// ----------------------------
app.patch('/api/goals/:goalId/frozen-decision',
  requireAuth,
  asyncHandler(async (req, res) => {
    const { goalId } = req.params;
    const uid = req.user.uid;
...
app.post('/api/save',
  ...
  requireOwnData,
  asyncHandler(async (req, res) => {
    const { uid, goalId, amount, phone, idempotencyKey } = req.body;
    ...
    if (goal.frozen) {
      return res.status(400).json({
        success: false,
        error: 'This goal is frozen because your subscription expired. Unlock it or renew to keep saving.',
        frozen: true
      });
    }
    ...
    const outcome = await withIdempotency(uid, idempotencyKey, async () => {
      ...
      if (isAirtelSuccess(result)) {
        ...
        await afterSuccessfulSave(goalBefore, updated);
        clearCache(`goals_${uid}`, `analytics_${uid}`);
        return { success: true, message: `MWK ${parsedAmount} saved`, reference, fee, goal: updated };
      } else {
        await logTransaction(uid, {
...
app.post('/api/withdraw',
  ...
  asyncHandler(async (req, res) => {
    ...
    // original result instead of disbursing twice.
    const outcome = await withIdempotency(uid, idempotencyKey, async () => {
      const reference = generateRef();
      ...
      if (isMockMode()) {
        ...
        clearCache(`goals_${uid}`, `analytics_${uid}`);
        return { success: true, mock: true, message: `MWK ${netPayout} sent`, reference, fee, netPayout };
      }
      ...
      if (isAirtelSuccess(result)) {
        ...
        clearCache(`goals_${uid}`, `analytics_${uid}`);
        return { success: true, message: `MWK ${netPayout} sent`, reference, fee, netPayout };
      } else {
        await logTransaction(uid, {
...
// merchant themselves.
// ----------------------------
app.get('/api/merchant/lookup/:code', requireAuth, asyncHandler(async (req, res) => {
...
app.post('/api/merchant/pay',
  ...
  asyncHandler(async (req, res) => {
    ...
    const outcome = await withIdempotency(uid, idempotencyKey, async () => {
      ...
      if (isMockMode()) {
        const txId = await logTransaction(uid, {
          type: 'merchant_payment', amount: parsedAmount, fee,
          feePercent: PLANS.business.transactionFeePercent,
          merchantUid: merchantDoc.id, merchantName, merchantCode,
          reference, status: 'mock', phone
        });
        await logFee(uid, { amount: fee, transactionId: txId, type: 'merchant_payment', plan: payer.plan || 'free' });
        ...
        return { success: true, mock: true, message: `MWK ${parsedAmount} paid to ${merchantName}`, reference, fee, merchantName };
      }
      ...
      if (isAirtelSuccess(result)) {
        ...
        clearCache(`analytics_${uid}`, `analytics_${merchantDoc.id}`);
        return { success: true, message: `MWK ${parsedAmount} paid to ${merchantName}`, reference, fee, merchantName };
      } else {
        await logTransaction(uid, {
...
// ----------------------------
app.post('/api/account/delete', requireAuth, rateLimit(3, 60 * 60 * 1000), asyncHandler(async (req, res) => {
  const uid = req.user.uid;
  ...
  res.json({ success: true, message: 'Your account has been closed. Transaction records are retained as required for financial record-keeping, with your personal details removed.' });
}));
...
// ----------------------------
async function checkFrozenGoalGracePeriod() {
  try {
...
async function unfreezeGoalsOnRenewal(uid) {
  const snap = await db.collection('goals')
    .where('uid', '==', uid)
    .where('frozen', '==', true)
    .where('completed', '==', false)
