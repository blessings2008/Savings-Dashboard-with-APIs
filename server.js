...
// ----------------------------
// AI CONFIGURATION
// Uses the Anthropic API for admin-facing intelligence features:
// chat assistant, anomaly detection, message drafting, insights,
// and error analysis. All AI features are admin-only (requireAdmin).
// ----------------------------
const AI = {
...
async function callClaude(systemPrompt, userMessage, maxTokens = 1024) {
  if (!AI.API_KEY) {
    throw new Error('AI features are not configured. Set ANTHROPIC_API_KEY in environment variables.');
  }
  const res = await fetch(AI.BASE_URL, {
    method: 'POST',
    headers: {
...
- Speak plainly, the founder is not a developer.`;
  ...
  try {
    const answer = await callClaude(systemPrompt, `${context}\n\nQuestion: ${conversationText}`, 600);
...
{"summary": "one sentence overview", "items": [{"uid": "...", "email":"...", "risk": "high|medium|low", "reason": "plain english explanation", "suggestedAction": "what to do"}]}`;
  ...
  try {
    const raw = await callClaude(systemPrompt, JSON.stringify(flagged.slice(0, 20)), 1200);
    let parsed;
    try {
    ...
    res.json({ success: true, anomalies: parsed.items || [], summary: parsed.summary || '' });
  } catch (err) {
...
Use MWK for currency. 4-6 short sentences total, no markdown headers, conversational but professional tone.`;
  ...
  try {
    const insight = await callClaude(systemPrompt, JSON.stringify(stats), 500);
    res.json({ success: true, insight, stats });
  } catch (err) {
    res.status(503).json({ success: false, error: err.message, stats });
  }
}));
...
Be concise. No jargon without explaining it. Format as a numbered list, one item per distinct error group.`;
  ...
  try {
    const analysis = await callClaude(systemPrompt, JSON.stringify(groupList), 1200);
    res.json({ success: true, analysis, groups: groupList.map(g => ({ source: g.source, message: g.message, count: g.count })) });
  } catch (err) {
    res.status(503).json({ success: false, error: err.message });
  }
}));
...
╚══════════════════════════════════════════════════════════════╝
  ...
  if (!AI.API_KEY) {
    console.log('ℹ️  ANTHROPIC_API_KEY is not set — admin AI features (chat assistant, anomaly detection, message drafting, insights, error analysis) will return a clear error until this is configured.');
  } else {
    console.log('✅ AI features configured — chat assistant, anomaly detection, message drafting, insights, and error analysis are live.');
  }
});
