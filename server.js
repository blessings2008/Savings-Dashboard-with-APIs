...
async function callAI(systemPrompt, userMessage, maxTokens = 1024) {
  ...
  if (!provider) {
    throw new Error('AI features are not configured. Set ANTHROPIC_API_KEY or GEMINI_API_KEY in environment variables.');
  }
...
{"summary": "one sentence overview", "items": [{"uid": "...", "email":"...", "risk": "high|medium|low", "reason": "plain english explanation", "suggestedAction": "what to do"}]}`;
  ...
  try {
    const raw = await callAI(systemPrompt, JSON.stringify(flagged.slice(0, 20)), 1200);
    let parsed;
    try {
...
app.get('/api/admin/ai/status', requireAdmin, asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    providerLabel: provider === 'anthropic' ? 'Claude (Anthropic)' : provider === 'gemini' ? 'Gemini (Google)' : null,
...
Use MWK for currency. 4-6 short sentences total, no markdown headers, conversational but professional tone.`;
  ...
  try {
    const insight = await callAI(systemPrompt, JSON.stringify(stats), 500);
...
Be concise. No jargon without explaining it. Format as a numbered list, one item per distinct error group.`;
  ...
  try {
    const analysis = await callAI(systemPrompt, JSON.stringify(groupList), 1200);
...
╚══════════════════════════════════════════════════════════════╝
  ...
  if (!activeProvider) {
    console.log('ℹ️  No AI provider configured — set ANTHROPIC_API_KEY or GEMINI_API_KEY to enable admin AI features (chat assistant, anomaly detection, message drafting, insights, error analysis).');
  } else {
    console.log(`✅ AI features configured — using ${activeProvider === 'anthropic' ? 'Anthropic (Claude)' : 'Google (Gemini)'}. Chat assistant, anomaly detection, message drafting, insights, and error analysis are live.`);
  }
});
