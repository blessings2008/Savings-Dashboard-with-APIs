...
// ----------------------------
// KYC: LEGACY SINGLE-STEP (kept for admin overrides)
// POST /api/kyc
// ----------------------------
app.post('/api/kyc',
  requireAuth,
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!AIRTEL.CLIENT_ID) {
      await db.collection('users').doc(uid).set({
        phone: phoneClean, phoneVerified: true,
        kycStatus: 'mock_verified',
        kycAt: FieldValue.serverTimestamp()
      }, { merge: true });
      clearCache(`plan_${uid}`, `profile_${uid}`);
      return res.json({ success: true, mock: true, verified: true, message: 'Phone verified (mock mode)' });
    }
    ...
    await db.collection('users').doc(uid).set({
      phone: phoneClean, phoneVerified: verified,
      kycStatus: verified ? 'verified' : 'failed',
      kycName: registeredName || null,
      kycAt: FieldValue.serverTimestamp()
    }, { merge: true });
    ...
    res.json({
      ...
      name: registeredName,
      message: verified ? `Verified. Registered as: ${registeredName}` : 'Verification failed'
    });
  })
);
...
app.patch('/api/goals/:goalId',
  ...
  asyncHandler(async (req, res) => {
    ...
    if (deadline) {
      ...
      updates.deadlinePassed = false;          // reset — new deadline is in the future
      updates.deadlineDecisionPending = false;  // clear any pending decision
    }
...
app.post('/api/roundup',
  ...
  asyncHandler(async (req, res) => {
    ...
    const parsed = parseFloat(spendAmount);
    const roundedUp = Math.ceil(parsed / 500) * 500; // round to nearest MWK 500
...
app.post('/api/macrodroid-proof', asyncHandler(async (req, res) => {
  ...
  await db.collection(collection).add({
    tid, successMessage,
    timestamp: FieldValue.serverTimestamp(),
    processed: false, source: 'macrodroid'
  });
...
// ============================================================
// MINI-APPS API
// Budget Tracker, School Fees, Loan Tracker,
// Bill Splitter, Merchant POS
// All use requireAuth + requireOwnData for security
// ============================================================
...
app.get('/api/budget/categories', requireAuth, asyncHandler(async (req, res) => {
  const uid = req.user.uid;
  const snap = await db.collection('budget_categories').where('uid', '==', uid).get();
  const cats = [];
  snap.forEach(doc => cats.push({ id: doc.id, ...doc.data() }));
  cats.sort((a, b) => toMillis(b.createdAt) - toMillis(a.createdAt));
  res.json({ success: true, categories: cats });
}));

// POST /api/budget/categories
app.post('/api/budget/categories', requireAuth, requireOwnData, asyncHandler(async (req, res) => {
  const { uid, name, emoji, monthlyLimit, color } = req.body;
  if (!name || !monthlyLimit) return res.status(400).json({ success: false, error: 'name and monthlyLimit required' });
  const ref = await db.collection('budget_categories').add({
    uid, name: sanitize(name), emoji: emoji || '💰',
...
app.patch('/api/budget/categories/:id', requireAuth, asyncHandler(async (req, res) => {
  const snap = await db.collection('budget_categories').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== req.user.uid) return res.status(403).json({ success: false, error: 'Forbidden' });
  const { name, emoji, monthlyLimit, color } = req.body;
  const updates = { updatedAt: FieldValue.serverTimestamp() };
  if (name) updates.name = sanitize(name);
  if (emoji) updates.emoji = emoji;
...
app.delete('/api/budget/categories/:id', requireAuth, asyncHandler(async (req, res) => {
  const snap = await db.collection('budget_categories').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== req.user.uid) return res.status(403).json({ success: false, error: 'Forbidden' });
...
app.post('/api/budget/expenses', requireAuth, requireOwnData, asyncHandler(async (req, res) => {
  const { uid, categoryId, amount, note, date } = req.body;
  if (!categoryId || !amount) return res.status(400).json({ success: false, error: 'categoryId and amount required' });
  const catSnap = await db.collection('budget_categories').doc(categoryId).get();
  if (!catSnap.exists || catSnap.data().uid !== uid) return res.status(403).json({ success: false, error: 'Category not found' });
  const ref = await db.collection('budget_expenses').add({
    uid, categoryId,
    categoryName: catSnap.data().name,
    categoryEmoji: catSnap.data().emoji,
...
// GET /api/budget/expenses?month=2026-06
app.get('/api/budget/expenses', requireAuth, asyncHandler(async (req, res) => {
  const uid = req.user.uid;
  const { month } = req.query; // e.g. "2026-06"
  const snap = await db.collection('budget_expenses').where('uid', '==', uid).get();
  let expenses = [];
...
app.delete('/api/budget/expenses/:id', requireAuth, asyncHandler(async (req, res) => {
  const snap = await db.collection('budget_expenses').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== req.user.uid) return res.status(403).json({ success: false, error: 'Forbidden' });
  await snap.ref.delete();
  res.json({ success: true });
}));
...
// GET /api/fees/plans
app.get('/api/fees/plans', requireAuth, asyncHandler(async (req, res) => {
  const uid = req.user.uid;
  const snap = await db.collection('school_fees_plans').where('uid', '==', uid).get();
  const plans = [];
  snap.forEach(doc => plans.push({ id: doc.id, ...doc.data() }));
  plans.sort((a, b) => toMillis(b.createdAt) - toMillis(a.createdAt));
  res.json({ success: true, plans });
}));

// POST /api/fees/plans
app.post('/api/fees/plans', requireAuth, requireOwnData, asyncHandler(async (req, res) => {
  const { uid, childName, school, termAmount, term, dueDate, emoji } = req.body;
  if (!childName || !termAmount || !dueDate) return res.status(400).json({ success: false, error: 'childName, termAmount and dueDate required' });
  const ref = await db.collection('school_fees_plans').add({
    uid, childName: sanitize(childName),
    school: sanitize(school || ''),
    termAmount: parseFloat(termAmount),
    term: sanitize(term || 'Term 1'),
    dueDate, emoji: emoji || '🎓',
    saved: 0, paid: false,
    createdAt: FieldValue.serverTimestamp()
  });
...
// POST /api/fees/plans/:id/save — add savings to a fees plan
app.post('/api/fees/plans/:id/save', requireAuth, asyncHandler(async (req, res) => {
  const uid = req.user.uid;
  const snap = await db.collection('school_fees_plans').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== uid) return res.status(403).json({ success: false, error: 'Forbidden' });
  const { amount } = req.body;
  if (!amount || parseFloat(amount) <= 0) return res.status(400).json({ success: false, error: 'Valid amount required' });
  const plan = snap.data();
  const newSaved = (plan.saved || 0) + parseFloat(amount);
  const paid = newSaved >= plan.termAmount;
  await snap.ref.update({ saved: newSaved, paid, updatedAt: FieldValue.serverTimestamp() });
  if (paid) {
    await pushNotification(uid, {
      type: 'fees_paid',
      topic: 'School Fees Reached!',
      message: `🎉 ${plan.childName}'s ${plan.term} fees target of MWK ${plan.termAmount.toLocaleString()} has been saved!`
    });
  }
  res.json({ success: true, saved: newSaved, paid });
}));

// PATCH /api/fees/plans/:id
app.patch('/api/fees/plans/:id', requireAuth, asyncHandler(async (req, res) => {
  const snap = await db.collection('school_fees_plans').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== req.user.uid) return res.status(403).json({ success: false, error: 'Forbidden' });
  const { childName, school, termAmount, term, dueDate, emoji } = req.body;
  const updates = { updatedAt: FieldValue.serverTimestamp() };
  if (childName) updates.childName = sanitize(childName);
  if (school) updates.school = sanitize(school);
  if (termAmount) updates.termAmount = parseFloat(termAmount);
  if (term) updates.term = sanitize(term);
...
app.delete('/api/fees/plans/:id', requireAuth, asyncHandler(async (req, res) => {
  const snap = await db.collection('school_fees_plans').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== req.user.uid) return res.status(403).json({ success: false, error: 'Forbidden' });
  await snap.ref.delete();
  res.json({ success: true });
...
// GET /api/loans
app.get('/api/loans', requireAuth, asyncHandler(async (req, res) => {
  const uid = req.user.uid;
  const snap = await db.collection('loans').where('uid', '==', uid).get();
  const loans = [];
  snap.forEach(doc => loans.push({ id: doc.id, ...doc.data() }));
  loans.sort((a, b) => toMillis(b.createdAt) - toMillis(a.createdAt));
  res.json({ success: true, loans });
}));

// POST /api/loans
app.post('/api/loans', requireAuth, requireOwnData, asyncHandler(async (req, res) => {
  const { uid, type, personName, personPhone, amount, note, dueDate } = req.body;
  if (!personName || !amount || !type) return res.status(400).json({ success: false, error: 'type, personName and amount required' });
  if (!['lent','borrowed'].includes(type)) return res.status(400).json({ success: false, error: 'type must be lent or borrowed' });
  const ref = await db.collection('loans').add({
    uid, type, personName: sanitize(personName),
    personPhone: sanitize(personPhone || ''),
    amount: parseFloat(amount),
...
app.post('/api/loans/:id/repay', requireAuth, asyncHandler(async (req, res) => {
  const uid = req.user.uid;
  const snap = await db.collection('loans').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== uid) return res.status(403).json({ success: false, error: 'Forbidden' });
  const { amount } = req.body;
  if (!amount || parseFloat(amount) <= 0) return res.status(400).json({ success: false, error: 'Valid amount required' });
  const loan = snap.data();
  const payment = Math.min(parseFloat(amount), loan.remaining);
  const newRemaining = Math.max(0, loan.remaining - payment);
  const settled = newRemaining === 0;
  await snap.ref.update({ remaining: newRemaining, settled, updatedAt: FieldValue.serverTimestamp() });
  await db.collection('loan_payments').add({
    uid, loanId: snap.id, amount: payment,
    timestamp: FieldValue.serverTimestamp()
  });
  if (settled) {
    await pushNotification(uid, {
      type: 'loan_settled',
      topic: 'Loan Settled',
      message: `✅ The ${loan.type === 'lent' ? 'loan to' : 'loan from'} ${loan.personName} of MWK ${loan.amount.toLocaleString()} has been fully settled.`
    });
  }
  res.json({ success: true, remaining: newRemaining, settled });
}));

// DELETE /api/loans/:id
app.delete('/api/loans/:id', requireAuth, asyncHandler(async (req, res) => {
  const snap = await db.collection('loans').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== req.user.uid) return res.status(403).json({ success: false, error: 'Forbidden' });
  await snap.ref.delete();
  res.json({ success: true });
...
app.get('/api/split/groups', requireAuth, asyncHandler(async (req, res) => {
  const uid = req.user.uid;
  const snap = await db.collection('bill_groups').where('uid', '==', uid).get();
  const groups = [];
...
app.post('/api/split/groups', requireAuth, requireOwnData, asyncHandler(async (req, res) => {
  const { uid, name, emoji, totalAmount, participants } = req.body;
  if (!name || !totalAmount || !participants?.length) {
    return res.status(400).json({ success: false, error: 'name, totalAmount and participants required' });
  }
  const perPerson = Math.ceil(parseFloat(totalAmount) / participants.length);
  ...
  const ref = await db.collection('bill_groups').add({
    uid, name: sanitize(name), emoji: emoji || '🧾',
    totalAmount: parseFloat(totalAmount),
    perPerson, participants: parts,
    settled: false,
    createdAt: FieldValue.serverTimestamp()
  });
  res.json({ success: true, id: ref.id, perPerson });
}));

// POST /api/split/groups/:id/mark-paid — mark one participant as paid
app.post('/api/split/groups/:id/mark-paid', requireAuth, asyncHandler(async (req, res) => {
  const uid = req.user.uid;
  const snap = await db.collection('bill_groups').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== uid) return res.status(403).json({ success: false, error: 'Forbidden' });
  const { participantIndex } = req.body;
  const group = snap.data();
  const parts = [...group.participants];
  if (participantIndex < 0 || participantIndex >= parts.length) return res.status(400).json({ success: false, error: 'Invalid participant index' });
  parts[participantIndex].paid = true;
  const settled = parts.every(p => p.paid);
  await snap.ref.update({ participants: parts, settled, updatedAt: FieldValue.serverTimestamp() });
  res.json({ success: true, settled });
}));

// POST /api/split/groups/:id/request-payment — send Airtel request to a participant
app.post('/api/split/groups/:id/request-payment', requireAuth, asyncHandler(async (req, res) => {
  const uid = req.user.uid;
  const snap = await db.collection('bill_groups').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== uid) return res.status(403).json({ success: false, error: 'Forbidden' });
  const { participantIndex } = req.body;
  ...
  if (!participant) return res.status(400).json({ success: false, error: 'Participant not found' });
  if (!participant.phone) return res.status(400).json({ success: false, error: 'No phone number for this participant' });

  const reference = generateRef();
  if (!AIRTEL.CLIENT_ID) {
    return res.json({ success: true, mock: true, message: `Payment request queued for ${participant.name}`, reference });
  }
  const result = await airtelCollect({ phone: participant.phone, amount: participant.share, reference });
  if (isAirtelSuccess(result)) {
    const parts = [...group.participants];
    parts[participantIndex].requestSent = true;
    await snap.ref.update({ participants: parts });
    res.json({ success: true, message: `USSD prompt sent to ${participant.name}`, reference });
  } else {
    res.status(400).json({ success: false, error: 'Payment request failed', details: result });
  }
}));

// DELETE /api/split/groups/:id
app.delete('/api/split/groups/:id', requireAuth, asyncHandler(async (req, res) => {
  const snap = await db.collection('bill_groups').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== req.user.uid) return res.status(403).json({ success: false, error: 'Forbidden' });
  await snap.ref.delete();
  res.json({ success: true });
...
app.get('/api/pos/products', requireAuth, asyncHandler(async (req, res) => {
  const uid = req.user.uid;
  const snap = await db.collection('pos_products').where('uid', '==', uid).get();
  const products = [];
...
app.post('/api/pos/products', requireAuth, requireOwnData, asyncHandler(async (req, res) => {
  const { uid, name, price, emoji, category, stock } = req.body;
  if (!name || !price) return res.status(400).json({ success: false, error: 'name and price required' });
  const ref = await db.collection('pos_products').add({
    uid, name: sanitize(name), price: parseFloat(price),
    emoji: emoji || '📦', category: sanitize(category || 'General'),
    stock: stock !== undefined ? parseInt(stock) : null, // null = unlimited
    createdAt: FieldValue.serverTimestamp()
  });
  res.json({ success: true, id: ref.id });
}));
...
app.patch('/api/pos/products/:id', requireAuth, asyncHandler(async (req, res) => {
  const snap = await db.collection('pos_products').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== req.user.uid) return res.status(403).json({ success: false, error: 'Forbidden' });
  const { name, price, emoji, category, stock } = req.body;
  const updates = { updatedAt: FieldValue.serverTimestamp() };
  if (name) updates.name = sanitize(name);
  if (price) updates.price = parseFloat(price);
  if (emoji) updates.emoji = emoji;
  if (category) updates.category = sanitize(category);
...
app.delete('/api/pos/products/:id', requireAuth, asyncHandler(async (req, res) => {
  const snap = await db.collection('pos_products').doc(req.params.id).get();
  if (!snap.exists || snap.data().uid !== req.user.uid) return res.status(403).json({ success: false, error: 'Forbidden' });
  await snap.ref.delete();
  res.json({ success: true });
...
// POST /api/pos/sales — process a sale
app.post('/api/pos/sales', requireAuth, requireOwnData, rateLimit(30, 60 * 1000), asyncHandler(async (req, res) => {
  const { uid, items, customerPhone, paymentMethod, note } = req.body;
  ...
  for (const item of items) {
    ...
    total += price * qty;
    saleItems.push({ ...item, qty, unitPrice: price, subtotal: price * qty });
    // Decrement stock if tracked
    if (item.productId) {
      const pSnap = await db.collection('pos_products').doc(item.productId).get();
      if (pSnap.exists && pSnap.data().stock !== null) {
        const newStock = Math.max(0, (pSnap.data().stock || 0) - qty);
        await pSnap.ref.update({ stock: newStock });
      }
    }
  }
  ...
  const reference = generateRef();
  const saleData = {
    uid, items: saleItems, total, customerPhone: sanitize(customerPhone || ''),
    paymentMethod: paymentMethod || 'airtel', note: sanitize(note || ''),
    reference, status: 'pending',
    createdAt: FieldValue.serverTimestamp()
  };

  // Process Airtel payment if phone provided
  if (customerPhone && paymentMethod !== 'cash') {
    if (!AIRTEL.CLIENT_ID) {
      saleData.status = 'mock';
    } else {
      const result = await airtelCollect({ phone: customerPhone, amount: total, reference });
      saleData.status = isAirtelSuccess(result) ? 'completed' : 'failed';
      saleData.airtelTxnId = result.txnId;
    }
  } else if (paymentMethod === 'cash') {
    saleData.status = 'completed';
  }

  const ref = await db.collection('pos_sales').add(saleData);

  // Log to platform_fees if completed
  if (['completed', 'mock'].includes(saleData.status)) {
    await logFee(uid, { amount: calcFee(total, 0.5), transactionId: ref.id, type: 'pos_sale', plan: 'pos' });
  }

  res.json({ success: true, id: ref.id, total, status: saleData.status, reference });
}));

// GET /api/pos/sales
app.get('/api/pos/sales', requireAuth, asyncHandler(async (req, res) => {
  const uid = req.user.uid;
  const { limit, date } = req.query;
  const snap = await db.collection('pos_sales').where('uid', '==', uid).get();
  let sales = [];
  snap.forEach(doc => sales.push({ id: doc.id, ...doc.data() }));
  if (date) sales = sales.filter(s => {
    const ms = toMillis(s.createdAt);
    return new Date(ms).toISOString().startsWith(date);
  });
  sales.sort((a, b) => toMillis(b.createdAt) - toMillis(a.createdAt));
  sales = sales.slice(0, parseInt(limit) || 50);
  const totalRevenue = sales.filter(s => s.status !== 'failed').reduce((sum, s) => sum + (s.total || 0), 0);
  res.json({ success: true, sales, totalRevenue });
}));

// ----------------------------
// HOME SCREEN: APP SUMMARY
// GET /api/apps/summary — quick stats for the home screen cards
// ----------------------------
app.get('/api/apps/summary', requireAuth, asyncHandler(async (req, res) => {
  const uid = req.user.uid;

  const [goalsSnap, budgetSnap, feesSnap, loansSnap, splitSnap, salesSnap] = await Promise.all([
    db.collection('goals').where('uid', '==', uid).where('completed', '==', false).get(),
    db.collection('budget_expenses').where('uid', '==', uid).get(),
    db.collection('school_fees_plans').where('uid', '==', uid).where('paid', '==', false).get(),
    db.collection('loans').where('uid', '==', uid).where('settled', '==', false).get(),
    db.collection('bill_groups').where('uid', '==', uid).where('settled', '==', false).get(),
    db.collection('pos_sales').where('uid', '==', uid).get(),
  ]);
  ...
  budgetSnap.forEach(doc => {
    const d = doc.data();
    if ((d.date || '').startsWith(thisMonth)) monthlySpend += d.amount || 0;
  });

  let totalLent = 0, totalBorrowed = 0;
  loansSnap.forEach(doc => {
    const d = doc.data();
    if (d.type === 'lent') totalLent += d.remaining || 0;
    else totalBorrowed += d.remaining || 0;
  });

  let posRevenue = 0;
  salesSnap.forEach(doc => {
    const d = doc.data();
    if (d.status !== 'failed' && (d.createdAt?._seconds * 1000 > Date.now() - 30 * 86400000)) {
      posRevenue += d.total || 0;
    }
  });
  ...
  res.json({
    success: true,
    summary: {
      savings: { activeGoals: goalsSnap.size },
      budget: { monthlySpend },
      fees: { pendingPlans: feesSnap.size },
      loans: { totalLent, totalBorrowed, activeLoans: loansSnap.size },
      split: { openGroups: splitSnap.size },
      pos: { monthRevenue: posRevenue, totalSales: salesSnap.size }
    }
  });
}));
