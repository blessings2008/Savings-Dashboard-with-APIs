...
app.post('/api/admin/messages', requireAdmin, asyncHandler(async (req, res) => {
  const { uid, message, type = 'admin_message' } = req.body;
  if (!message?.trim()) return res.status(400).json({ success: false, error: 'message required' });

  if (uid) {
    // Single user
    await pushNotification(uid, { type, message: message.trim() });
    await db.collection('admin_messages').add({
      uid, message: message.trim(), type,
  ...
  for (const user of list.users) {
    ...
    batch.set(notifRef, {
      uid: user.uid, type, message: message.trim(),
  ...
  await db.collection('admin_messages').add({
    uid: null, message: message.trim(), type,
