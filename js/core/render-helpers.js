    ...
    <div class="goal-card">
      ...
      ${!compact ? `
        <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">
          <button class="btn btn-primary btn-sm" data-action="allocate" data-goal="${g.id}" ${g.frozen || g.completed ? "disabled" : ""} ${g.frozen ? "title=\"Frozen — subscription expired\"" : g.completed ? "title=\"Goal already complete\"" : ""}>Allocate from Balance</button>
          ${(() => {
            // "Move to Balance" — the reverse of allocate. Available
            // anytime on a flexible goal; only once complete on a
            // locked goal, since that's the entire point of a hard
            // lock (money committed until the target is reached).
            const canDeallocate = !g.frozen && (g.lockType !== 'hard' || g.completed) && (g.saved || 0) > 0;
            const lockedReason = g.lockType === 'hard' && !g.completed ? "title=\"Locked until goal is complete\"" : "";
            return `<button class="btn btn-outline btn-sm" data-action="deallocate" data-goal="${g.id}" ${canDeallocate ? "" : "disabled"} ${g.frozen ? "title=\"Frozen — subscription expired\"" : lockedReason}>Move to Balance</button>`;
          })()}
        </div>
      ` : ""}
    </div>
