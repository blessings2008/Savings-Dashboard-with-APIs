...
export async function renderAccountPage(main, navigate) {
  ...
  main.innerHTML = `
    <div class="page active">
      ...
      <div class="card">
        <div class="card-header"><div class="card-title">Delete Account</div></div>
        <p style="font-size:12.5px;color:var(--muted);margin-bottom:14px">
          Closing your account disables it and removes your personal details. You have 30 days to
          restore it just by signing in again — after that, restoring it needs support. Transaction
          records are kept, as required for financial record-keeping.
        </p>
        <button class="btn btn-outline" id="acc-delete" style="width:100%">Delete My Account</button>
      </div>
    </div>
  `;
  ...
  document.getElementById("acc-delete").onclick = () => openDeleteAccountModal(navigate);
}

// ----------------------------
// DELETE MY ACCOUNT (self-service)
// Mirrors the admin-initiated delete flow's export-before-confirm
// shape: the Delete button stays disabled until the user has
// actually downloaded their export in this modal session, same
// enforcement as the admin panel version — not just documented as
// recommended, actually required by the UI.
// ----------------------------
function openDeleteAccountModal(navigate) {
  ...
  root.innerHTML = `
    <div class="modal">
      ...
      <div class="auth-error" style="display:block;background:rgba(244,63,94,0.08);border-color:rgba(244,63,94,0.25);color:var(--red)">
        This disables your login and removes your personal details. You can restore your account by
        signing in again within 30 days — after that, restoring it requires contacting support.
        Transaction records are retained as required for financial record-keeping.
      </div>

      <div class="input-group">
        <label class="input-label">Step 1 — download a copy of your data first</label>
        <button class="btn btn-outline" id="del-export-btn" style="width:100%">⬇️ Export My Data</button>
      </div>

      <div class="input-group">
        <label class="input-label">Step 2 — type DELETE to confirm</label>
      ...
      <div class="modal-actions">
        ...
        <button class="btn btn-danger" id="del-submit" disabled title="Export your data first">🗑️ Delete Account</button>
      </div>
    </div>
  `;
  ...
  root.querySelector("#del-export-btn").onclick = async () => {
    const btn = document.getElementById("del-export-btn");
    btn.disabled = true; btn.innerHTML = `<span class="spinner"></span>`;
    try {
      ...
      a.href = url;
      a.download = `pocketvault-my-data-export.json`;
      ...
      btn.disabled = false; btn.textContent = "⬇️ Export My Data";
    }
  };
  ...
  submitBtn.onclick = async () => {
    ...
    if (!hasExported) { errBox.style.display = "block"; errBox.textContent = "Export your data first"; return; }
