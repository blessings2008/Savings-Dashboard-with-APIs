...
export async function renderMerchantPage(main, navigate) {
  ...
  main.innerHTML = `
    <div class="page active">
      ...
      <div class="balance-hero">
        <div class="balance-label">Merchant Account Balance</div>
        <div class="balance-amount">MWK <span>${fmt(state.user?.accountBalance || 0)}</span></div>
        <div class="balance-meta">Spendable now — includes collections, code payments, and balance payments received</div>
        <div class="balance-icon-actions">
          <button class="bal-icon-btn primary" id="m-withdraw-btn">
      ...
      <div class="grid-3">
        <div class="merchant-stat">
          ...
          <div class="lbl">Lifetime Revenue (MWK)</div>
        </div>
        <div class="merchant-stat">
  ...
  document.getElementById("m-withdraw-btn")?.addEventListener("click", () => openWithdrawModal(undefined, navigate));
