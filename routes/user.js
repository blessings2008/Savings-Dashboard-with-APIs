...
import {
  ...
  logSystemError, sendExternalAlert, REFERRAL_BONUS_MWK, log
} from '../helpers.js';
  ...
  asyncHandler(async (req, res) => {
    ...
    const outcome = await withIdempotency(uid, idempotencyKey, async () => {
      ...
      if (isMockMode()) {
        ...
        clearCache(`profile_${uid}`, `analytics_${uid}`);
        return { success: true, mock: true, message: `MWK ${netPayout} sent`, reference, fee: fee.total, netPayout };
      }
      ...
      if (isAirtelSuccess(result)) {
        ...
        clearCache(`profile_${uid}`, `analytics_${uid}`);
        return { success: true, message: `MWK ${netPayout} sent`, reference, fee: fee.total, netPayout };
      } else {
        await logTransaction(uid, {
  ...
  asyncHandler(async (req, res) => {
    ...
    const outcome = await withIdempotency(uid, idempotencyKey, async () => {
      ...
      await pushNotification(uid, {
        ...
        message: `💸 Sent MWK ${parsedAmount.toLocaleString()} to ${merchantName} (internal transfer, no Airtel fee).`
      });
      await pushNotification(merchantDoc.id, {
        type: 'internal_transfer_received',
        message: `💰 Received MWK ${netToMerchant.toLocaleString()} via internal transfer.`
      });
