    ...
    try {
      ...
      panel.innerHTML = `
        <div class="card" style="border-color:rgba(0,229,160,0.25)">
          <div class="card-hdr"><div class="card-title">✦ AI Error Analysis</div></div>
          <div class="card-body">
            <div style="white-space:pre-wrap;font-size:13px;line-height:1.7;color:var(--text)">${escapeHTML(res.analysis)}</div>
          </div>
        </div>`;
    } catch (e) {
      panel.innerHTML = `<div class="card"><div class="card-body"><div class="empty-state"><p>${escapeHTML(e.message)}</p></div></div></div>`;
    }
...
async function renderAIAssistant(main) {
  ...
  main.innerHTML = `
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-hdr">
        <div class="card-title">📊 This Week's Insights</div>
  ...
  async function sendChat() {
    ...
    try {
      ...
      aiChatHistory.push({ role: "assistant", content: `⚠️ ${e.message}` });
    }
    log.innerHTML = aiChatHistory.map(m => chatBubble(m)).join("");
    log.scrollTop = log.scrollHeight;
    btn.disabled = false;
  ...
  document.getElementById("scan-btn").onclick = async () => {
    ...
    try {
      ...
      if (anomalies.length === 0) {
        results.innerHTML = `<div class="card"><div class="card-body"><div class="empty-state">
