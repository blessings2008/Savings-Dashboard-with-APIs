...
async function renderUserDetailPage(main, uid) {
  ...
  document.getElementById("send-msg-btn").onclick = () => {
    ...
    root.querySelector("#dm-send").onclick = async () => {
      ...
      btn.disabled = true; btn.innerHTML = '<span class="spinner"></span>';
      try {
        await apiCall("/api/admin/messages", "POST", { uid, message });
        closeModal();
        toast("Message sent");
      } catch (e) {
...
async function renderMessagesPage(main) {
  ...
  main.innerHTML = `
    <div class="page active">
      ...
      <div class="card">
        ...
        <button class="btn btn-primary" id="msg-send" style="width:100%">Send Message</button>
      </div>
  ...
  document.getElementById("msg-send").onclick = async () => {
    ...
    btn.disabled = true; btn.innerHTML = '<span class="spinner"></span>';
    try {
      const res = await apiCall("/api/admin/messages", "POST", { uid, message });
