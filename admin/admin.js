...
function renderLogin() {
  ...
  async function doLogin() {
    if (btn.disabled) return; // guard against Enter + click both firing
...
async function navigate(page, param) {
  ...
  try {
    switch (page) {
      ...
      case "notifications_admin": await renderNotificationsAdmin(main, param); break;
      case "support": await renderSupport(main, param); break;
...
async function renderUserDetail(main, uid) {
  ...
  main.innerHTML = `
    <div class="page-hdr">
      ...
      <div class="page-hdr-right">
        ...
        <button class="btn btn-outline btn-sm" id="export-btn">⬇️ Export Data</button>
        ${user.accountDeleted ? `
          <button class="btn btn-primary btn-sm" id="restore-btn">♻️ Restore Account</button>
        ` : `
          <button class="btn ${user.disabled ? "btn-primary" : "btn-danger"} btn-sm" id="suspend-btn">
            ${user.disabled ? "✅ Restore" : "🚫 Suspend"}
          </button>
          <button class="btn btn-danger btn-sm" id="delete-btn">🗑️ Delete Account</button>
        `}
      </div>
    </div>

    ${user.accountDeleted ? `
      <div class="auth-error" style="display:block;background:rgba(245,158,11,0.08);border-color:rgba(245,158,11,0.25);color:var(--amber);margin-bottom:16px">
        This account was deleted${user.deletedByAdmin ? " by an admin" : " by the user"}${user.deletionReason ? ` — reason: "${escapeHTML(user.deletionReason)}"` : ""}.
        ${user.purgeDeadline ? `Unresolved balance will be swept on ${new Date(user.purgeDeadline).toLocaleDateString()} if not restored before then.` : ""}
      </div>
    ` : ""}
  ...
  document.getElementById("export-btn").onclick = () => downloadUserExport(uid);
  document.getElementById("delete-btn")?.addEventListener("click", () => openDeleteAccountModal(uid, user.email));
  document.getElementById("restore-btn")?.addEventListener("click", async () => {
    if (!confirm("Restore this deleted account? This brings back login access and clears the deletion status.")) return;
    const btn = document.getElementById("restore-btn"); btn.disabled = true;
    try {
      const res = await apiCall(`/api/admin/users/${uid}/restore`, "POST");
      toast(res.message || "Account restored");
...
// ----------------------------
// DOWNLOAD A USER'S DATA EXPORT
// Fetches the export JSON via apiCall (which already attaches the
// admin secret header) and triggers a client-side Blob download —
// the backend's Content-Disposition header only matters for a
// direct browser navigation, which apiCall's fetch() call is not,
// so the download has to be built here instead.
// ----------------------------
async function downloadUserExport(uid) {
  try {
    const data = await apiCall(`/api/admin/users/${uid}/export`);
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `pocketvault-data-export-${uid}.json`;
...
// Mirrors openSendMessageModal's structure. Forces the export to
// happen first (the download button is right there in the modal,
// and the confirm button stays disabled until the admin has
// triggered it at least once this modal session) — the same
// export-before-delete guarantee the user-facing flow gives users
// for their own accounts, now enforced for admin-initiated deletion
// too rather than just documented as a recommended step.
// ----------------------------
function openDeleteAccountModal(uid, email) {
  ...
  root.innerHTML = `
    <div class="modal">
      ...
      <p class="modal-sub">Deleting: <strong>${escapeHTML(email||uid)}</strong></p>
      <div class="auth-error" style="display:block;background:rgba(244,63,94,0.08);border-color:rgba(244,63,94,0.25);color:var(--red)">
        This disables login and removes personal details. Transaction and fee records are retained for financial record-keeping and cannot be recovered afterward for the parts that are removed.
      </div>
      <div class="input-group">
        <label class="input-label">Step 1 — download a copy of their data first</label>
        <button class="btn btn-outline" id="del-export-btn" style="width:100%">⬇️ Export Data</button>
      </div>
      <div class="input-group">
        <label class="input-label">Step 2 — reason for deletion (required, kept in the audit log)</label>
        <textarea class="input" id="del-reason" rows="2" style="resize:vertical" placeholder="e.g. User requested closure via support ticket #1234"></textarea>
      </div>
      <div class="input-group">
        <label class="input-label">Step 3 — type DELETE to confirm</label>
        <input class="input" id="del-confirm" placeholder="DELETE">
      </div>
      <div id="del-error" class="auth-error" style="display:none"></div>
      <div class="modal-actions">
        <button class="btn btn-outline" id="del-cancel">Cancel</button>
        <button class="btn btn-danger" id="del-submit" disabled title="Export the data first">🗑️ Delete Account</button>
      </div>
    </div>`;
  root.classList.add("open");
  root.querySelector("#del-close").onclick = root.querySelector("#del-cancel").onclick = closeModal;
  root.addEventListener("click", e => { if (e.target === root) closeModal(); });

  const submitBtn = root.querySelector("#del-submit");
  root.querySelector("#del-export-btn").onclick = async () => {
    await downloadUserExport(uid);
    hasExported = true;
  ...
  submitBtn.onclick = async () => {
    const confirmation = document.getElementById("del-confirm").value.trim();
    const reason = document.getElementById("del-reason").value.trim();
    const errBox = document.getElementById("del-error");
    if (!hasExported) { errBox.style.display = "block"; errBox.textContent = "Export the data first"; return; }
    if (!reason) { errBox.style.display = "block"; errBox.textContent = "A reason is required"; return; }
    if (confirmation !== "DELETE") { errBox.style.display = "block"; errBox.textContent = 'Type DELETE exactly to confirm'; return; }
    errBox.style.display = "none";
    submitBtn.disabled = true; submitBtn.innerHTML = `<span class="spinner"></span>`;
    try {
      await apiCall(`/api/admin/users/${uid}/delete`, "POST", { confirmation: "DELETE", reason });
      closeModal();
      toast("Account deleted");
      navigate("users");
    } catch (e) {
      errBox.style.display = "block"; errBox.textContent = e.message;
      submitBtn.disabled = false; submitBtn.textContent = "🗑️ Delete Account";
    }
  };
}
...
// ----------------------------
async function renderNotificationsAdmin(main, param) {
  const typeFilter = param?.type || "";
  const query = typeFilter ? `?type=${encodeURIComponent(typeFilter)}` : "";
  const res = await apiCall(`/api/admin/notifications${query}`);
  const notifications = res.notifications || [];
  const types = res.types || [];

  main.innerHTML = `
    <div class="page-hdr">
      <div class="page-hdr-left"><h2>Notifications Feed</h2><p>${res.total || 0} total across all users${typeFilter ? ` · filtered: ${escapeHTML(typeFilter)}` : ""}</p></div>
      <div class="page-hdr-right">
        <button class="btn btn-outline btn-sm" onclick="navigate('notifications_admin')">↻ Refresh</button>
      </div>
    </div>
    <div class="card">
      <div class="card-body" style="padding-bottom:0">
        <select class="input" id="notif-type-filter" style="max-width:280px;margin-bottom:14px">
          <option value="">All types</option>
          ${types.map(t => `<option value="${escapeHTML(t)}" ${t === typeFilter ? "selected" : ""}>${escapeHTML(t)}</option>`).join("")}
        </select>
      </div>
      ${notifications.length === 0
        ? `<div class="card-body"><div class="empty-state"><div class="empty-icon">🔔</div><p>No notifications found</p></div></div>`
        : `<div class="table-wrap"><table>
            <thead><tr><th>User</th><th>Type</th><th>Message</th><th>Time</th></tr></thead>
            <tbody>
              ${notifications.map(n => `<tr>
                <td class="mono truncate" style="max-width:140px">${escapeHTML(n.uid || "—")}</td>
                <td><span class="badge badge-warn">${escapeHTML(n.type || "—")}</span></td>
                <td style="max-width:320px"><div class="truncate" style="font-size:13px">${escapeHTML(n.message || "—")}</div></td>
                <td class="mono">${timeAgo(n.timestamp)}</td>
              </tr>`).join("")}
            </tbody>
          </table></div>`}
  ...
  document.getElementById("notif-type-filter").onchange = (e) => {
    navigate("notifications_admin", { type: e.target.value });
  };
}

// ----------------------------
// CUSTOMER CARE — SUPPORT QUEUE
// Two views in one function: the queue list (default), and a thread
// detail view when param.threadId is set. Chat threads are visually
// distinguished from tickets since chat implies an admin should be
// responsive right now — a ticket has no such expectation.
// ----------------------------
async function renderSupport(main, param) {
  if (param?.threadId) return renderSupportThread(main, param.threadId);

  const modeFilter = param?.mode || "";
  const statusFilter = param?.status ?? "";
  const qs = [];
  if (modeFilter) qs.push(`mode=${encodeURIComponent(modeFilter)}`);
  if (statusFilter) qs.push(`status=${encodeURIComponent(statusFilter)}`);
  const res = await apiCall(`/api/admin/support/threads${qs.length ? "?" + qs.join("&") : ""}`);
  const threads = res.threads || [];

  main.innerHTML = `
    <div class="page-hdr">
      <div class="page-hdr-left"><h2>Customer Support</h2><p>${threads.length} thread${threads.length !== 1 ? "s" : ""}</p></div>
      <div class="page-hdr-right">
        <button class="btn btn-outline btn-sm" onclick="navigate('support')">↻ Refresh</button>
      </div>
    </div>
    <div class="card">
      <div class="card-body" style="padding-bottom:0;display:flex;gap:10px;flex-wrap:wrap">
        <select class="input" id="support-mode-filter" style="max-width:180px">
          <option value="">All modes</option>
          <option value="chat" ${modeFilter === "chat" ? "selected" : ""}>💬 Chat only</option>
          <option value="ticket" ${modeFilter === "ticket" ? "selected" : ""}>🎫 Tickets only</option>
        </select>
        <select class="input" id="support-status-filter" style="max-width:180px">
      ...
      ${threads.length === 0
        ? `<div class="card-body"><div class="empty-state"><div class="empty-icon">💬</div><p>No threads found</p></div></div>`
        : `<div class="table-wrap"><table>
            <thead><tr><th>User</th><th>Mode</th><th>Last message</th><th>Status</th><th>Updated</th><th></th></tr></thead>
            <tbody>
              ${threads.map(t => {
                const lastMsg = t.messages?.[t.messages.length - 1];
                const statusBadge = t.status === "resolved" ? "badge-resolved" : t.status === "in_progress" ? "badge-pending" : "badge-warn";
                return `<tr>
                  <td class="mono truncate" style="max-width:140px">${escapeHTML(t.userName || t.uid || "—")}</td>
                  <td>${t.mode === "chat" ? `<span class="badge badge-pending">💬 Chat</span>` : `<span class="badge">🎫 Ticket</span>`}</td>
                  <td style="max-width:280px"><div class="truncate" style="font-size:13px">${escapeHTML(lastMsg?.text || "—")}</div></td>
                  <td><span class="badge ${statusBadge}">${escapeHTML(t.status)}</span></td>
                  <td class="mono">${timeAgo(t.lastMessageAt)}</td>
                  <td><button class="btn btn-primary btn-sm" data-open-thread="${t.id}">Open</button></td>
                </tr>`;
              }).join("")}
            </tbody>
          </table></div>`}
  ...
  document.getElementById("support-mode-filter").onchange = (e) => {
    navigate("support", { mode: e.target.value, status: statusFilter });
  };
  document.getElementById("support-status-filter").onchange = (e) => {
    navigate("support", { mode: modeFilter, status: e.target.value });
  };
  main.querySelectorAll("[data-open-thread]").forEach(btn => {
    btn.addEventListener("click", () => navigate("support", { threadId: btn.dataset.openThread }));
  });
}

async function renderSupportThread(main, threadId) {
  const res = await apiCall(`/api/admin/support/threads/${threadId}`);
  const thread = res.thread;
  if (!thread) { navigate("support"); return; }

  main.innerHTML = `
    <div class="page-hdr">
      <div class="page-hdr-left">
        <button class="btn btn-outline btn-sm" onclick="navigate('support')" style="margin-bottom:10px">← Back to queue</button>
        <h2>${thread.mode === "chat" ? "💬 Chat" : "🎫 Ticket"} — ${escapeHTML(thread.userName || thread.uid)}</h2>
        <p>${thread.mode === "chat" ? "Live — reply promptly" : "Async — reply when reviewed"}</p>
      </div>
      <div class="page-hdr-right">
        <select class="input" id="thread-status" style="max-width:180px">
    ...
    ${thread.relatedGoalId ? `
      <div class="card" style="margin-bottom:12px">
        <p style="font-size:12.5px;color:var(--muted)">
          This conversation references a specific goal (ID: <span class="mono">${escapeHTML(thread.relatedGoalId)}</span>).
          Open the user's profile to view or act on it directly.
        </p>
        <button class="btn btn-outline btn-sm" style="margin-top:8px" onclick="navigate('userDetail', '${thread.uid}')">View User →</button>
      </div>
    ` : ""}

    <div class="card" style="display:flex;flex-direction:column;gap:10px;max-height:50vh;overflow-y:auto" id="thread-messages">
      ${(thread.messages || []).map(m => `
        <div style="align-self:${m.from === "admin" ? "flex-end" : "flex-start"};max-width:80%">
          <div style="font-size:10.5px;color:var(--muted);margin-bottom:2px;text-align:${m.from === "admin" ? "right" : "left"}">
            ${m.from === "admin" ? "Support" : escapeHTML(thread.userName || "User")}
          </div>
          <div style="background:${m.from === "admin" ? "var(--green)" : "var(--surface2)"};color:${m.from === "admin" ? "#04120c" : "var(--text)"};padding:8px 12px;border-radius:12px;font-size:13.5px">
            ${escapeHTML(m.text)}
          </div>
        </div>
      `).join("")}
    ...
    <div style="display:flex;gap:8px;margin-top:14px">
      <input class="input" id="thread-reply-input" placeholder="Type a reply…" style="flex:1">
      <button class="btn btn-primary" id="thread-reply-send">Send</button>
    </div>
  `;
  ...
  document.getElementById("thread-status").onchange = async (e) => {
    try {
      await apiCall(`/api/admin/support/threads/${threadId}`, "PATCH", { status: e.target.value });
  ...
  sendBtn.onclick = async () => {
    ...
    try {
      await apiCall(`/api/admin/support/threads/${threadId}/messages`, "POST", { message: text });
      input.value = "";
      navigate("support", { threadId });
    } catch (err) {
      toast(err.message, "error");
    } finally {
...
async function renderOperations(main) {
  ...
  main.innerHTML = `
    ...
    <div class="card">
      <div class="card-hdr">
        <div><div class="card-title">Backups</div><div class="card-sub">On-demand Firestore export to Cloud Storage</div></div>
      </div>
      <div class="card-body">
        <p style="font-size:12.5px;color:var(--muted);margin-bottom:10px">
          This triggers a manual export in addition to whatever recurring schedule is configured in GCP
          (see docs/BACKUPS.md for setup). Exports run asynchronously on Google's side and can take a
          few minutes for a large database.
        </p>
        <div style="display:flex;gap:8px;margin-bottom:14px">
          <input class="input" id="backup-bucket" placeholder="gs://your-backup-bucket/firestore-backups" style="flex:1">
          <button class="btn btn-primary btn-sm" id="backup-trigger-btn">⬇️ Export Now</button>
        </div>
        <div id="backup-history-wrap"><div class="empty-state"><p>Loading history…</p></div></div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-hdr">
        <div><div class="card-title">Unresolved Funds</div><div class="card-sub">Unclaimed balances swept from deleted accounts — not platform revenue</div></div>
      </div>
      <div class="card-body">
        <p style="font-size:12.5px;color:var(--muted);margin-bottom:10px">
          When a deleted account isn't restored within 60 days, any remaining account balance lands here
          rather than being treated as earned revenue. Resolve an entry once you've reconciled it —
          e.g. paying the original user back after identity verification, or writing it off.
        </p>
        <div id="unresolved-funds-wrap"><div class="empty-state"><p>Loading…</p></div></div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-hdr">
        <div><div class="card-title">Flagged Referrals</div><div class="card-sub">Held for review — referrer and referred user share a device fingerprint</div></div>
      </div>
      <div class="card-body">
        <p style="font-size:12.5px;color:var(--muted);margin-bottom:10px">
          No bonus has been paid on these yet. A shared device can be a genuine self-referral attempt,
          or something innocent like a shared family device — review before approving or denying.
        </p>
        <div id="flagged-referrals-wrap"><div class="empty-state"><p>Loading…</p></div></div>
      </div>
    </div>
  `;
  ...
  document.getElementById("backup-trigger-btn").onclick = async () => {
    const bucketUri = document.getElementById("backup-bucket").value.trim();
    if (!bucketUri.startsWith("gs://")) { toast("Enter a valid gs:// bucket URI", "error"); return; }
    const btn = document.getElementById("backup-trigger-btn");
    btn.disabled = true; btn.textContent = "Starting…";
    try {
      await apiCall("/api/admin/backups/export", "POST", { bucketUri });
      toast("Export started — check history below for status");
      loadBackupHistory();
    } catch (e) {
      toast(e.message, "error");
    } finally {
      btn.disabled = false; btn.textContent = "⬇️ Export Now";
    }
  };
...
async function loadFlaggedReferrals() {
  const wrap = document.getElementById("flagged-referrals-wrap");
  if (!wrap) return;
  try {
    const res = await apiCall("/api/admin/referrals/flagged");
    const referrals = res.referrals || [];
    wrap.innerHTML = referrals.length === 0
      ? `<div class="empty-state"><p>No flagged referrals</p></div>`
      : `<div class="table-wrap"><table>
          <thead><tr><th>Referrer</th><th>Referred</th><th>Flagged</th><th></th></tr></thead>
          <tbody>${referrals.map(r => `<tr>
            <td class="mono truncate" style="max-width:140px">${escapeHTML(r.referrerUid || "—")}</td>
            <td class="mono truncate" style="max-width:140px">${escapeHTML(r.referredUid || "—")}</td>
            <td class="mono">${timeAgo(r.flaggedAt)}</td>
            <td style="display:flex;gap:6px">
              <button class="btn btn-primary btn-sm" data-approve-ref="${r.id}">Approve</button>
              <button class="btn btn-outline btn-sm" data-deny-ref="${r.id}">Deny</button>
            </td>
          </tr>`).join("")}</tbody>
        </table></div>`;

    wrap.querySelectorAll("[data-approve-ref]").forEach(btn => {
      btn.addEventListener("click", async () => {
        if (!confirm("Approve this referral? Both bonuses will be paid.")) return;
        btn.disabled = true;
        try {
          await apiCall(`/api/admin/referrals/${btn.dataset.approveRef}`, "PATCH", { action: "approve" });
          toast("Referral approved — bonuses paid");
          loadFlaggedReferrals();
        } catch (e) { toast(e.message, "error"); btn.disabled = false; }
      });
    });
    wrap.querySelectorAll("[data-deny-ref]").forEach(btn => {
      btn.addEventListener("click", async () => {
        if (!confirm("Deny this referral? No bonus will be paid.")) return;
        btn.disabled = true;
        try {
          await apiCall(`/api/admin/referrals/${btn.dataset.denyRef}`, "PATCH", { action: "deny" });
          toast("Referral denied");
          loadFlaggedReferrals();
        } catch (e) { toast(e.message, "error"); btn.disabled = false; }
      });
    });
  } catch {
    wrap.innerHTML = `<div class="empty-state"><p>Couldn't load flagged referrals</p></div>`;
  }
}

async function loadUnresolvedFunds() {
  const wrap = document.getElementById("unresolved-funds-wrap");
  if (!wrap) return;
  try {
    const res = await apiCall("/api/admin/unresolved-funds");
    const entries = res.entries || [];
    const totalLine = `<div style="font-weight:700;margin-bottom:10px">Total unresolved: MWK ${fmt(res.totalUnresolved || 0)}</div>`;
    wrap.innerHTML = totalLine + (entries.length === 0
      ? `<div class="empty-state"><p>No unresolved funds on record</p></div>`
      : `<div class="table-wrap"><table>
          <thead><tr><th>User</th><th>Amount</th><th>Swept</th><th>Status</th><th></th></tr></thead>
          <tbody>${entries.map(e => `<tr>
            <td class="mono truncate" style="max-width:160px">${escapeHTML(e.uid || "—")}</td>
            <td>MWK ${fmt(e.amount || 0)}</td>
            <td class="mono">${timeAgo(e.sweptAt)}</td>
            <td>${e.resolved
              ? `<span class="badge badge-ok">Resolved</span>`
              : `<span class="badge badge-warn">Unresolved</span>`}</td>
            <td>${e.resolved ? "" : `<button class="btn btn-outline btn-sm" data-resolve-fund="${e.id}">Mark Resolved</button>`}</td>
          </tr>`).join("")}</tbody>
        </table></div>`);

    wrap.querySelectorAll("[data-resolve-fund]").forEach(btn => {
      btn.addEventListener("click", async () => {
        const resolution = prompt("Resolution note (e.g. 'Paid back via [method] on [date]'):");
        if (resolution === null) return; // cancelled
        btn.disabled = true;
        try {
          await apiCall(`/api/admin/unresolved-funds/${btn.dataset.resolveFund}`, "PATCH", { resolution });
    ...
    wrap.innerHTML = `<div class="empty-state"><p>Couldn't load unresolved funds</p></div>`;
  }
}
...
async function loadBackupHistory() {
  ...
  if (!wrap) return;
  try {
    const res = await apiCall("/api/admin/backups/history");
    const exportsList = res.exports || [];
    wrap.innerHTML = exportsList.length === 0
      ? `<div class="empty-state"><p>No manual exports triggered yet</p></div>`
      : `<div class="table-wrap"><table>
          <thead><tr><th>Bucket</th><th>Status</th><th>Triggered</th></tr></thead>
          <tbody>${exportsList.map(e => `<tr>
            <td class="mono truncate" style="max-width:220px">${escapeHTML(e.bucketUri || "—")}</td>
            <td>${e.status === "started" ? '<span class="badge badge-warn">Started</span>' : escapeHTML(e.status || "—")}</td>
            <td class="mono">${timeAgo(e.triggeredAt)}</td>
          </tr>`).join("")}</tbody>
        </table></div>`;
  } catch {
    wrap.innerHTML = `<div class="empty-state"><p>Couldn't load export history</p></div>`;
  }
}
...
async function renderAIAssistant(main) {
  ...
  // Load persisted chat history once per admin session (Improvement #5)
  if (!aiChatHistoryLoaded) {
    try {
      const res = await apiCall("/api/admin/ai/chat-history");
      aiChatHistory = res.history || [];
    } catch { aiChatHistory = []; }
  ...
  main.innerHTML = `
    <div class="page-hdr">
      <div class="page-hdr-left"><h2>✦ AI Assistant</h2><p>Ask questions, get insights, and let it take actions on your confirmation</p></div>
      <div class="page-hdr-right">
        ...
        <button class="btn btn-outline btn-sm" id="view-action-log">📋 Action Log</button>
      </div>
    </div>
    ...
    ${!statusData.configured ? `
      <div class="card" style="margin-bottom:16px;border-color:rgba(245,158,11,0.3)">
        <div class="card-body">
          <p style="font-size:13px;color:var(--amber)">⚠ No AI provider is configured. Set <span class="mono">ANTHROPIC_API_KEY</span>, <span class="mono">GEMINI_API_KEY</span>, or <span class="mono">GROQ_API_KEY</span> in Render environment variables to enable this page.</p>
        </div>
      </div>
    ` : ""}
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-hdr">
        <div class="card-title" id="insights-title">📊 This Week's Insights ${insightData?.cached ? `<span class="badge" style="background:rgba(14,165,233,0.12);color:var(--blue);margin-left:6px;font-size:9px">CACHED</span>` : ""}</div>
    ...
    <div class="card">
      <div class="card-hdr">
        ...
        ${aiChatHistory.length > 0 ? `<button class="btn btn-ghost btn-sm" id="clear-chat">Clear History</button>` : ""}
      </div>
      <div class="card-body">
        <div id="chat-log" style="max-height:440px;overflow-y:auto;display:flex;flex-direction:column;gap:12px;margin-bottom:14px">
          ${aiChatHistory.length === 0 ? `
            <div class="empty-state">
              <div class="empty-icon">✦</div>
              <p>Ask things like "which users are close to reaching a goal?" or "remind unverified users to complete KYC"</p>
            </div>
          ` : aiChatHistory.map(m => chatBubble(m)).join("")}
        </div>
        <div style="display:flex;gap:8px">
          <input class="input" id="chat-input" placeholder="Ask a question, or ask it to take an action..." style="flex:1">
  ...
  document.getElementById("refresh-insights").onclick = async () => {
    ...
    const cardTitle = document.getElementById("insights-title");
  ...
  document.getElementById("view-action-log").onclick = () => openActionLogModal();

  document.getElementById("clear-chat")?.addEventListener("click", async () => {
    if (!confirm("Clear the assistant's conversation history? This can't be undone.")) return;
    aiChatHistory = [];
    try { await apiCall("/api/admin/ai/chat-history", "DELETE"); } catch {}
    navigate("ai_assistant");
  });

  function reasoningToggle(thinking) {
    if (!thinking?.trim()) return "";
    const id = `think_${Math.random().toString(36).slice(2, 9)}`;
    return `
      <div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,0.08)">
        <button type="button" onclick="document.getElementById('${id}').style.display = document.getElementById('${id}').style.display==='none'?'block':'none'; this.textContent = this.textContent.includes('Show') ? '▾ Hide reasoning' : '▸ Show reasoning'"
          style="background:none;border:none;color:var(--muted);font-size:11px;cursor:pointer;padding:0;font-weight:600">▸ Show reasoning</button>
        <div id="${id}" style="display:none;margin-top:6px;font-size:11.5px;color:var(--muted);line-height:1.6;white-space:pre-wrap;font-style:italic">${escapeHTML(thinking)}</div>
      </div>`;
  }
    ...
    if (m.actionProposal) {
      return `
        <div style="display:flex;justify-content:flex-start">
          <div style="max-width:88%;padding:12px 14px;border-radius:14px;font-size:13px;line-height:1.6;
            background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.25)">
            <div style="font-size:10px;font-weight:800;color:var(--amber);margin-bottom:6px">⚡ ACTION PROPOSED — CONFIRMATION NEEDED</div>
            <div style="color:var(--text);margin-bottom:10px">${escapeHTML(m.content)}</div>
            ${m.executed ? `
              <div style="font-size:12px;color:var(--green);font-weight:600">✅ Executed</div>
            ` : `
              <div style="display:flex;gap:8px">
                <button class="btn btn-primary btn-sm" data-confirm-action="${m.id}">✓ Confirm & Execute</button>
                <button class="btn btn-outline btn-sm" data-cancel-action="${m.id}">Cancel</button>
              </div>
            `}
            ${reasoningToggle(m.thinking)}
          </div>
        </div>`;
    }
    return `
      <div style="display:flex;${isUser ? "justify-content:flex-end" : "justify-content:flex-start"}">
        <div style="max-width:80%;padding:10px 14px;border-radius:14px;font-size:13px;line-height:1.6;
          ...
          ${!isUser ? reasoningToggle(m.thinking) : ""}
        </div>
      </div>`;
  }
  ...
  function bindActionButtons() {
    document.querySelectorAll("[data-confirm-action]").forEach(btn => {
      btn.addEventListener("click", () => confirmAction(btn.dataset.confirmAction));
    });
    document.querySelectorAll("[data-cancel-action]").forEach(btn => {
      btn.addEventListener("click", () => cancelAction(btn.dataset.cancelAction));
    });
  }

  async function persistHistory() {
    try { await apiCall("/api/admin/ai/chat-history", "POST", { history: aiChatHistory }); } catch {}
  }

  async function confirmAction(msgId) {
    const msg = aiChatHistory.find(m => m.id === msgId);
    if (!msg?.actionProposal) return;
    const btns = document.querySelectorAll(`[data-confirm-action="${msgId}"], [data-cancel-action="${msgId}"]`);
    btns.forEach(b => b.disabled = true);
    try {
      await apiCall("/api/admin/ai/execute-action", "POST", {
        action: msg.actionProposal.action,
        args: msg.actionProposal.args,
      });
      msg.executed = true;
      aiChatHistory.push({ role: "assistant", content: "✅ Done — that action has been executed." });
      renderLog();
      await persistHistory();
      toast("Action executed");
    } catch (e) {
      aiChatHistory.push({ role: "assistant", content: `⚠️ That action failed: ${e.message}` });
      renderLog();
      await persistHistory();
    }
  }

  function cancelAction(msgId) {
    aiChatHistory = aiChatHistory.filter(m => m.id !== msgId);
    aiChatHistory.push({ role: "assistant", content: "No problem — cancelled. Let me know if you'd like something else." });
    renderLog();
    persistHistory();
  }
  ...
  async function sendChat() {
    ...
    renderLog();
    const log = document.getElementById("chat-log");
    log.innerHTML += `<div style="display:flex;justify-content:flex-start"><div style="padding:10px 14px;border-radius:14px;background:var(--surface2);border:1px solid var(--border)"><span class="spinner"></span></div></div>`;
    log.scrollTop = log.scrollHeight;
    input.value = "";
    btn.disabled = true;
    ...
    try {
      const res = await apiCall("/api/admin/ai/chat", "POST", {
        question,
        history: aiChatHistory.slice(-10, -1).filter(m => !m.actionProposal)
      });

      if (res.actionProposal) {
        aiChatHistory.push({
          role: "assistant",
          content: res.actionProposal.summary || res.answer,
          actionProposal: res.actionProposal,
          thinking: res.actionProposal.thinking || null,
          id: `action_${Date.now()}`,
        });
      } else {
        aiChatHistory.push({ role: "assistant", content: res.answer, thinking: res.thinking || null });
      }
    } catch (e) {
...
// ----------------------------
// AI ACTION LOG MODAL
// (Improvement #6 — audit trail of everything the AI has
// actually executed, kept separate from normal admin activity)
// ----------------------------
async function openActionLogModal() {
  ...
  root.innerHTML = `
    <div class="modal" style="max-width:600px">
      ...
      <p class="modal-sub">Every action the AI assistant has actually executed, most recent first</p>
      <div id="action-log-body" style="max-height:400px;overflow-y:auto">
        <div class="loading-row"><span class="spinner"></span></div>
      </div>
    </div>`;
  root.classList.add("open");
  root.querySelector("#log-close").onclick = closeModal;
  root.addEventListener("click", e => { if (e.target === root) closeModal(); });

  try {
    const res = await apiCall("/api/admin/ai/action-log");
    const actions = res.actions || [];
    const body = document.getElementById("action-log-body");
    if (actions.length === 0) {
      body.innerHTML = `<div class="empty-state"><div class="empty-icon">📋</div><p>No AI-executed actions yet</p></div>`;
      return;
    }
    body.innerHTML = actions.map(a => `
      <div style="padding:12px 0;border-bottom:1px solid var(--border)">
        <div class="flex-between" style="margin-bottom:4px">
          <span class="badge" style="background:rgba(168,85,247,0.12);color:var(--purple)">${escapeHTML(a.actionType)}</span>
          <span class="mono" style="font-size:11px">${timeAgo(a.executedAt)}</span>
        </div>
        <div class="mono" style="font-size:11px;color:var(--muted);white-space:pre-wrap">${escapeHTML(JSON.stringify(a.details, null, 2))}</div>
      </div>
    `).join("");
  } catch (e) {
    document.getElementById("action-log-body").innerHTML = `<div class="empty-state"><p>${escapeHTML(e.message)}</p></div>`;
  }
}
...
async function pollBadges() {
  ...
  try {
    const res = await apiCall("/api/admin/support/threads?status=open");
    const openCount = (res.threads || []).length;
    document.querySelectorAll("[data-page='support']").forEach(el => {
      el.querySelectorAll(".nav-badge-count").forEach(b => b.remove());
      if (openCount > 0) {
        const b = document.createElement("span");
        b.className = "nav-badge-count";
        b.textContent = openCount > 99 ? "99+" : openCount;
        el.appendChild(b);
      }
    });
  } catch {}
