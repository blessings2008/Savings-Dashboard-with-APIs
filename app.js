...
// ----------------------------
// LOAD USER PROFILE (kycStatus, phone, streaks, etc.)
// Merges backend profile data onto the Firebase auth user object
// in state.user, so every page can read state.user.kycStatus,
// state.user.phone, state.user.streakCount, etc. without each
// page needing its own fetch.
// ----------------------------
...
async function renderDashboardPage(main) {
      ...
      <div class="balance-hero">
        ...
        <div class="balance-icon-actions">
          <button class="bal-icon-btn primary" id="btn-save-quick">
            ...
            <span class="bal-icon-btn-label">Save</span>
          </button>
  ...
  document.getElementById("qa-pay-merchant")?.addEventListener("click", () => openPayMerchantModal());
}
    ...
    <div class="goal-card">
      ...
      ${!compact ? `
        <div style="margin-top:12px;display:flex;gap:8px">
          <button class="btn btn-primary btn-sm" data-action="save" data-goal="${g.id}" ${g.frozen ? "disabled title=\"Frozen — subscription expired\"" : ""}>Add Savings</button>
...
// Rough day-count for the frozen-goal grace countdown, e.g. "3 days".
// Deliberately coarse (no hours) — this is a soft nudge shown on a
// card, not a precise timer.
function daysUntil(timestampMs) {
...
function txRowHTML(tx) {
  const isOut = ["savings", "roundup", "subscription", "withdrawal", "collection", "disbursement", "merchant_payment"].includes(tx.type)
    && ["savings", "roundup", "subscription", "merchant_payment"].includes(tx.type) === false;
...
async function renderGoalsPage(main) {
  ...
  if (newlyFrozen) openFrozenGoalPromptModal(newlyFrozen);
  ...
  main.querySelectorAll('[data-frozen-decision="unlock"]').forEach(btn => {
    btn.addEventListener("click", () => handleFrozenDecision(btn.dataset.goal, "unlock"));
  });
  main.querySelectorAll('[data-frozen-decision="keep_locked"]').forEach(btn => {
    btn.addEventListener("click", () => handleFrozenDecision(btn.dataset.goal, "keep_locked"));
  });
}
...
async function handleFrozenDecision(goalId, action) {
  try {
...
function openFrozenGoalPromptModal(goal) {
...
function openSaveModal(preselectGoalId) {
  ...
  root.querySelector("#save-submit").onclick = async () => {
    ...
    btn.disabled = true; btn.innerHTML = `<span class="spinner"></span>`;
    try {
      const res = await api.save(state.user.uid, { goalId, amount, phone, idempotencyKey });
      closeModal();
...
function openWithdrawModal(preselectGoalId) {
  ...
  root.querySelector("#wd-submit").onclick = async () => {
    ...
    btn.disabled = true; btn.innerHTML = `<span class="spinner"></span>`;
    try {
      const res = await api.withdraw(state.user.uid, { goalId, amount, phone, idempotencyKey });
      closeModal();
...
function ruleHTML(r) {
  ...
  let desc = "";
  if (r.type === "weekly") desc = `Weekly (${r.schedule || "Mon"}) · MWK ${fmt(r.amount)} → ${goal?.name || "goal"}`;
  else if (r.type === "monthly") desc = `Monthly (day ${r.schedule || "1"}) · MWK ${fmt(r.amount)} → ${goal?.name || "goal"}`;
  else if (r.type === "income_percent") desc = `${r.percent}% of MWK ${fmt(r.declaredIncome)} on day ${r.incomeDay} → ${goal?.name || "goal"}`;
