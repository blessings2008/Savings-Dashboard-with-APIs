...
function renderShell(user) {
  ...
  let sidebarHTML = `<div class="logo">Pocket<span>Vault</span></div>`;
  ...
  document.getElementById("app").innerHTML = `
    ...
    <div class="bottom-nav">
      ...
      <div class="nav-item" data-page="account">
        <span class="nav-icon">${initials(user)}</span>Account
      </div>
    </div>
  ...
  main.innerHTML = `
    <div class="page active">
      ...
      <div class="card">
        ...
        <div class="input-group">
          <label class="input-label">Airtel Money number</label>
          <input class="input" id="acc-phone" type="tel" placeholder="e.g. 0991234567" value="${state.user.profilePhone || ""}">
        </div>
...
function openKYCModal(phone) {
  ...
  root.innerHTML = `
    <div class="modal">
      ...
      <p class="modal-sub">We'll confirm ${escapeHTML(phone)} is registered with Airtel Money.</p>
      <div id="kyc-error" class="auth-error" style="display:none"></div>
      <div class="modal-actions">
        <button class="btn btn-outline" id="kyc-cancel">Cancel</button>
        <button class="btn btn-primary" id="kyc-confirm">Verify</button>
      </div>
    </div>
  `;
  root.classList.add("open");
  ...
  root.querySelector("#kyc-confirm").onclick = async () => {
    const errBox = document.getElementById("kyc-error");
    const btn = root.querySelector("#kyc-confirm");
    btn.disabled = true; btn.innerHTML = `<span class="spinner"></span>`;
    try {
      const res = await api.kyc(state.user.uid, phone);
      if (res.success) {
        state.user.kycStatus = res.mock ? "mock_verified" : "verified";
