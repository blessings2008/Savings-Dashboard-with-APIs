...
function addHeader(doc, title, subtitle) {
  ...
  doc.moveTo(42, 148).lineTo(570, 148).strokeColor('#e5e7eb').stroke(); doc.y = 165;
}
function addFooter(doc) { const page = doc.page; doc.fontSize(8).font('Helvetica').fillColor('#94a3b8').text(`PocketVault • Generated ${fmtDate(new Date())}`, 42, page.height - 36, { align: 'left' }); }
function addTable(doc, headers, rows, widths) {
  const x = 42, rowHeight = 24, totalWidth = widths.reduce((a, b) => a + b, 0); let y = doc.y;
  const drawHeader = () => { doc.rect(x, y, totalWidth, rowHeight).fill('#f1f5f9'); let cx = x; headers.forEach((header, i) => { doc.fillColor('#334155').font('Helvetica-Bold').fontSize(8).text(String(header), cx + 6, y + 8, { width: widths[i] - 12, ellipsis: true }); cx += widths[i]; }); y += rowHeight; };
  drawHeader();
  rows.forEach((row, rowIndex) => {
    if (y > doc.page.height - 62) { doc.addPage(); addFooter(doc); y = 48; drawHeader(); }
    if (rowIndex % 2 === 1) doc.rect(x, y, totalWidth, rowHeight).fill('#fafafa'); let cx = x;
    row.forEach((cell, i) => { doc.fillColor('#334155').font('Helvetica').fontSize(8).text(String(cell ?? '—'), cx + 6, y + 8, { width: widths[i] - 12, ellipsis: true }); cx += widths[i]; }); y += rowHeight;
  });
  doc.y = y + 16;
}
...
export function buildMonthlyAdminPdf({ monthLabel, metrics, transactions = [] }) {
  return buildPdf(doc => {
    ...
    const cards = [['Users', metrics.users], ['New users', metrics.newUsers], ['Transactions', metrics.transactions], ['Transaction volume', fmtMoney(metrics.volume)], ['Savings volume', fmtMoney(metrics.savings)], ['Withdrawals', fmtMoney(metrics.withdrawals)], ['Revenue', fmtMoney(metrics.revenue)], ['Failed transactions', metrics.failed]];
    let x = 42, y = doc.y;
    cards.forEach(([label, value], i) => { if (i && i % 2 === 0) { x = 42; y += 68; } doc.roundedRect(x, y, 252, 54, 8).fill('#f8fafc').strokeColor('#e2e8f0').stroke(); doc.fillColor('#64748b').font('Helvetica').fontSize(8).text(label, x + 12, y + 10); doc.fillColor('#111827').font('Helvetica-Bold').fontSize(15).text(String(value), x + 12, y + 27); x += 276; });
...
function buildPdf(draw) {
  const doc = new PDFDocument({ size: 'A4', margin: 42, info: { Title: 'PocketVault Report', Author: 'PocketVault' } }); const chunks = [];
  return new Promise((resolve, reject) => { doc.on('data', chunk => chunks.push(chunk)); doc.on('end', () => resolve(Buffer.concat(chunks))); doc.on('error', reject); try { draw(doc); addFooter(doc); doc.end(); } catch (error) { reject(error); } });
}
