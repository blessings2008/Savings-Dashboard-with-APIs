...
router.post('/api/support/threads/:threadId/ai-reply',requireAuth,asyncHandler(async(req,res)=>{
  ...
  // Do not put serverTimestamp() inside arrayUnion(): Firestore does not allow
  // field transforms nested inside an array element. Use a normal ISO timestamp
  // for the message and keep the server timestamp on the thread itself.
  const message={from:'ai',text,createdAt:new Date().toISOString(),ai:true};
  await ref.update({
