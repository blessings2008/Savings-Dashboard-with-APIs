...
async function getUserEmail(uid) { const user = await adminAuth.getUser(uid); if (!user.email) { const error = new Error('No email address is associated with this account.'); error.statusCode = 400; throw error; } return { user, email: user.email }; }
...
router.post('/api/email/transaction', requireAuth, rateLimit(10, 60 * 60 * 1000), asyncHandler(async (req, res) => {
  const uid = req.user.uid; const { type, amount, status = 'completed', reference } = req.body; const allowedTypes = new Set(['savings', 'withdrawal', 'subscription', 'merchant_payment']);
  if (!allowedTypes.has(type)) return res.status(400).json({ success: false, error: 'Invalid transaction type' });
  const parsedAmount = Number(amount); if (!Number.isFinite(parsedAmount) || parsedAmount <= 0) return res.status(400).json({ success: false, error: 'Invalid amount' });
...
router.get('/api/reports/transactions.pdf', requireAuth, rateLimit(10, 60 * 60 * 1000), asyncHandler(async (req, res) => {
  ...
  res.set({ 'Content-Type': 'application/pdf', 'Content-Disposition': 'attachment; filename="pocketvault-transaction-history.pdf"', 'Cache-Control': 'private, no-store' }); res.send(pdf);
}));
...
router.post('/api/admin/reports/monthly/email', requireAdmin, rateLimit(3, 24 * 60 * 60 * 1000), asyncHandler(async (req, res) => {
  const now = new Date(); let year = Number(req.body?.year || now.getFullYear()); let month = Number(req.body?.month || now.getMonth());
  ...
  const result = await sendMonthlyAdminReport({ year, month, to: req.body?.to || process.env.ADMIN_REPORT_EMAIL });
