...
async function withTransactionPin(path, body) {
  const status = await request("GET", "/api/security/transaction-pin/status");
  if (!status.configured) {
    const newPin = await transactionPinModal({ setup: true });
    await request("POST", "/api/security/transaction-pin/set", { pin: newPin });
    return request("POST", path, { ...body, transactionPin: newPin });
  }

  const pin = await transactionPinModal();
  return request("POST", path, { ...body, transactionPin: pin });
}
...
export const api = {
  ...
  transactionPinStatus: () => request("GET", "/api/security/transaction-pin/status"), setTransactionPin: (pin, currentPin) => request("POST", "/api/security/transaction-pin/set", { pin, ...(currentPin ? { currentPin } : {}) }), verifyTransactionPin: (pin) => request("POST", "/api/security/transaction-pin/verify", { pin })
};
