...
.pv-otp-card{width:min(440px,100%);background:var(--surface,#fff);border:1px solid rgba(15,23,42,.08);border-radius:24px;padding:30px;box-shadow:0 24px 70px rgba(15,23,42,.10);text-align:center}.pv-otp-icon{width:64px;height:64px;margin:0 auto 18px;border-radius:20px;display:grid;place-items:center;background:#ecfdf5;font-size:30px}.pv-otp-card h1{margin:0 0 8px;font-size:26px}.pv-otp-muted{color:var(--muted,#64748b);font-size:14px;line-height:1.6;margin:0 0 22px}.pv-otp-email{font-weight:700;color:var(--text,#111827)}.pv-otp-input{width:100%;box-sizing:border-box;text-align:center;letter-spacing:12px;font-size:28px;font-weight:800;padding:16px 10px;border:1px solid #d8e0dc;border-radius:14px;background:var(--surface2,#f8faf9);outline:none}.pv-otp-input:focus{border-color:#00a86b;box-shadow:0 0 0 4px rgba(0,168,107,.10)}.pv-otp-msg{min-height:20px;margin:12px 0;font-size:13px}.pv-otp-error{color:#dc2626}.pv-otp-ok{color:#15803d}.pv-otp-actions{display:flex;gap:10px;margin-top:16px}.pv-otp-actions button{flex:1}.pv-otp-note{font-size:11px;color:#94a3b8;margin-top:18px;line-height:1.5}@media(max-width:480px){.pv-otp-card{padding:24px;border-radius:20px}.pv-otp-input{letter-spacing:8px}}
</style>`;
...
export function renderEmailOtp(user, onVerified) {
  ...
  const code = document.getElementById('pv-otp-code'); const verify = document.getElementById('pv-otp-verify'); const resend = document.getElementById('pv-otp-resend'); const msg = document.getElementById('pv-otp-msg');
  ...
  verify.onclick = async () => {
    const value = code.value.replace(/\D/g, ''); if (value.length !== 6) return show('Enter the 6-digit code from your email.');
  ...
  code.addEventListener('keydown', e => { if (e.key === 'Enter') verify.click(); }); resend.onclick = send;
